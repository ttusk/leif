...
- `src/domain` — entities and pure domain logic (Contest, Subject, Topic, Resource, StudySession, StudyRecord, CycleState, Mural)
...
- Markdown schema 2 invariants: a unique `leif-id` per entity, exactly one H1 = canonical name, ordered wikilink lists own sequence, folder placement owns containment, no IDs in filenames or wikilinks, unknown content preserved losslessly. See `docs/leif-markdown.md`.
