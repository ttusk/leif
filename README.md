...
Leif is a plugin for [Obsidian](https://obsidian.md/) that organizes exam-prep study in a single panel. It brings together concursos, matérias, assuntos, recursos, sessões, and registros to show what to study now, and since 3.0 it stores everything as **open Markdown** in your vault.

## Features

- **Hoje** (Today): what to study now, following the active concurso's cycle. Recording a sessão and advancing the cycle happen in one gesture.
...
- **Registros** (Records): the history of every sessão, grouped and editable as a whole.
- **Mural** (Wall): readable Markdown reference area for the concurso — notes, links, and snapshots.

## Open Markdown, schema 2

In Leif 3.0, study content lives as readable Markdown under `Leif/concursos/<concurso>/`. Small files represent concursos, matérias, assuntos, recursos, sessões, registros, and the mural. You can edit them in Obsidian, in another editor, or with an AI agent — with or without Obsidian running. Schema 2 is the only writable study authority; the previous mixed JSON/Markdown model is gone.
...
Enable Leif under **Community plugins** and open the panel from the ribbon icon or the **Abrir painel** command. Create a concurso, organize its matérias and recursos, record each sessão, and check **Hoje** to follow the cycle.

The command palette offers:

- **Abrir painel**, **Abrir Hoje**, **Nova sessão de estudo**, **Registrar estudo recomendado**, **Avançar ciclo sem registrar**
